...
# Example 3: Handler with explicit WorkflowContext output
# -------------------------------------------------------
#
# When you need to control the type of message an executor emits, fully typing the
# handler signature (both the message parameter and the WorkflowContext) is sufficient
# because the framework infers the executor's input and output types from those
# annotations. This keeps the decorator simple while still allowing precise typing.


class ExclamationAdder(Executor):
    """Executor that adds exclamation marks while keeping handler typing explicit.

    Annotating both the incoming message and the WorkflowContext[str] lets the
    framework infer this handler's input and output types without decorator arguments.
    """
    ...
    @handler
    async def add_exclamation(self, message: str, ctx: WorkflowContext[str]) -> None:
        """Add exclamation marks to the input.

        The message parameter is typed as str and the WorkflowContext[str] signals
        that the handler emits str outputs, so the decorator stays parameterless.
        """
        result = f"{message}!!!"
        await ctx.send_message(result)
...
def create_workflow() -> Workflow:
    ...
    # Build the graph by first setting the start executor, then wiring the edges.
    return (
        WorkflowBuilder()
